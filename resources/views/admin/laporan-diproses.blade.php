@extends('layouts.diproses')

@section('content')
    <h2>Halaman Laporan Diproses</h2>
        ...
        <tbody class="text-center">
            @forelse($reports as $report)
                <tr>
                    ...
                    <td>
                        <!-- Ubah status laporan -->
                        <form action="{{ route('reports.updateStatus', ['id' => $report->id, 'newStatus' => 'Sedang Diproses']) }}" method="POST" style="display:inline;">
    @csrf
    @method('PATCH')
    <button type="submit" class="btn btn-warning btn-sm">Proses</button>
            ...
            @empty
                <tr>
                    <td colspan="6">Tidak ada laporan dengan status diproses.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
